...
//  implicit private val mode: Mode = Example
  ...
  def sumNext(target: Long, contigious: Seq[Long], pointer: Int): Option[Seq[Long]] = {
    ...
    else sumNext(target, contigious :+ inputLongs(mode)(pointer), pointer + 1)
  }

  def main(args: Array[String]): Unit = {
    println(s"AOC 2020 - Day $day")

    val part1 = inputLongs.view.zipWithIndex
      .drop(preambleLength)
      .find { x =>
        val preamble = inputLongs.slice(x._2 - preambleLength, x._2)
    ...
    val part2: Option[Long] = inputLongs.indices
